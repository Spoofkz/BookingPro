...
export async function POST(_: NextRequest, routeContext: RouteContext) {
  ...
  if (template.status !== ScheduleTemplateStatus.ACTIVE) {
  ...
  await prisma.auditLog.create({
    data: {
      ...
      action: 'schedule.template_activated',
      entityType: 'schedule_template',
      entityId: template.id,
    },
  })

  return NextResponse.json({ ok: true, status: ScheduleTemplateStatus.ACTIVE })
}
